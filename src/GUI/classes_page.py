...
from src.utils.utils import load_graph, get_all_classes
...
class ClassesPage(tk.Frame):
    ...
    def display_page(self):
        ...
        if not classes:
            self.listbox.insert(tk.END, "No classes found")
            self.next_btn.config(state=tk.DISABLED)
        else:
            for cls in classes:
                self.listbox.insert(tk.END, cls)
            if len(classes) < self.page_size:
                self.next_btn.config(state=tk.DISABLED)
            else:
    ...
    def show_graph(self, event):
        ...
        self.controller.show_graph(class_name, self.db_name)
